...
Fork of the MessageBird Go REST API client (`github.com/Bijles-aan-Huis-B-V/messagebird-go-rest-api`). Provides a Go SDK for MessageBird's SMS, Voice, Conversations, MMS, WhatsApp, and other APIs.
...
**Multiple API roots**: The default endpoint is `rest.messagebird.com`. Packages with different API roots (Voice, Conversations, Integrations) prepend their own base URL to bypass the default:
...
**Error handling divergence**: The `voice` package has its own `ErrorResponse`/`Error` types (with `Code` + `Message` fields), registered via `messagebird.SetErrorReader()` in an `init()` function. All other packages use `messagebird.ErrorResponse` (with `Code` + `Description` + `Parameter` fields).
